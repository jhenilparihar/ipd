...
from models.docter import Docter as DocterModel
import json
import bcrypt
...
class Signup(Resource):
    ...
    def post(self):
        ...
        if docter == "true":
            parser.add_argument("registration_number", type=str, required=True, help="registration_number is required")
            ...
            response = DocterModel.add_docter(args)
            if response["error"]:
                return response, 500
            
            docter = response["data"]
            access_token = create_access_token(identity=docter.registration_number, expires_delta=False)

            return {"error": False, "data": json.loads(docter.to_json()), "access_token": access_token}
...
class Login(Resource):
    ...
    def post(self):
        ...
        if docter == "true":
            ...
            response = DocterModel.get_docter_by_registration_number(args["registration_number"])
            if response["error"]:
                return response, 404
            ...
            passwordMatch = bcrypt.checkpw(
                args["password"].encode("utf-8"), docter.password.encode("utf-8")
            )
